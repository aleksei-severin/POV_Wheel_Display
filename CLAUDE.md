...
**Stopping the slideshow stops the display too.** `OP_ALBUM`(stop)/`/album?action=stop` used to only clear `slideshowActive`, leaving whatever file (or effect) was last showing keep playing — so stopping a slideshow took two presses: one to stop the cycling, another to actually blank the rim. That's exactly what `OP_STOP`/`/stop` already did (`slideshowActive = false; force_stop_display = true;`, plus clearing a running effect through `fileLoaderTask`), so the two are now the same call: `stopDisplayAndSlideshow()`. A synced pair (Android app, phone-driven — see below) needs the same guarantee from the app side instead, since a phone-driven pair never sets `slideshowActive` on-device at all: `WheelVm.stopSyncedSlideshow()` sends `OP_STOP` to *both* wheels in the group before tearing down the sync, not just the one the user tapped Stop on.
...
- **A vibration sensor still LOW after that 2 s window is not dropped from the wake mask forever.** It almost never means the switch is broken — far more often the wheel is simply resting in an orientation where the ball sits on the contact (on its side, say), which can persist for as long as it sits there. Excluding it outright would mean "always wakes by shake unless put to sleep by a button hold or app Off" stops being true the moment the wheel is stored on the wrong side — silently, since the exclusion used to be a single log line nobody sees while the device is already asleep. `armWakeSourcesAndSleep(false)` arms the button alone plus a bare timer wakeup every `VIB_STUCK_RECHECK_S` (20 s, `config.h`) instead, and sets `vib_stuck_recheck` (RTC memory) so `setup()` can tell a routine recheck apart from a real wake: woken by that timer with the pin still LOW, it goes straight back to sleep the same way — no flash writes, no LittleFS/BLE/Wi-Fi init, just the GPIO hold + `esp_sleep_enable_ext1_wakeup`/`esp_sleep_enable_timer_wakeup` pair — so the cost of "wrong orientation" is a few ms of wake every 20 s, not a wheel that silently stopped answering to anything but the button. The moment the pin reads HIGH again (picked up, reoriented) the check falls through to a normal boot, and the *next* `enterDeepSleep()` tries to arm vibration normally again.
