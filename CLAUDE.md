...
- `cal` (whether the six-sensor anchor is live) says whether the six-sensor anchor is live at all — until calibration converges only sensor 0 anchors, so error accumulates over a whole revolution instead of a sixth of one, and the image wobbles once per revolution rather than six times. The peak PLL residual (phase jitter, degrees) and the per-sensor offsets are internal state used only to drive the anchor itself now — see the note at the end of this section for why they're no longer surfaced anywhere.
...
4. `fillSectorIntoBuffer()` reads the PSRAM `frameBuffer` and builds the SK9822 DMA frame. `sector0` is **fractional**: each LED mixes the two neighbouring 1° source sectors by the fractional part, so an edge lands at a sub-degree angle instead of snapping to a whole degree. Arm `N` renders `sector0 ± 60°·N`; the sign is `global_arm_reverse` (see below). `mirror_back_face == true` feeds the **LED 44–87 face** the mirrored angle `540 - bf` while the LED 0–43 face (the one on the first-LED side of this hardware) keeps `bf` — the reflection cancels the "seen from the far side" flip so text reads correctly from both sides, **regardless of spin direction**: `bf`/`bm` each make their own face self-consistent (correct when viewed square-on from that face's own normal), which is a fact of the diode's geometry, not of which way it spins or which side of the bike it faces — so `Speed`/`Clock` and any file with the checkbox ticked need no further handling below. Which physical face is which is a hardware fact, found by testing on a real wheel: Clock and Speed came out mirror-reversed on *both* faces at once, which means the two roles (not just one side) were swapped — `bf`/`bm` are swapped in `fillSectorIntoBuffer` to fix it. If a future hardware revision reverses it again, the symptom to watch for is the same: both faces wrong together points at a swapped role assignment, one face wrong points at something else. The flag is **per source**, published in the blanked swap window next to `frame_fmt` and re-derived on every load — never persisted: `loadFrameFromFile()` takes it from bit 15 of the ANI6 frame-count word (see Frame Buffer Format), and `effectsStart()` sets it `true` only for `Speed` and `Clock` (their text must read from both sides). Upgrading to this firmware therefore un-mirrors every animation already on the device and the four non-text effects, which were previously mirrored unconditionally — re-upload an animation with the **Mirror back face** checkbox ticked to restore a two-sided-readable file; legacy `ANI5`/`ANIM` stills stay unmirrored for good.
   - **Both faces sample the same single reference angle by default** (`mirror_back_face == false`) — they light the same pixels for a given rotor position, so the image is identical on both sides and one side reads mirror-reversed (fine for a symmetric picture or logo, and the common case: everything except `Speed`/`Clock`/checkbox-marked files). **That shared reference angle is `bf` on forward spin but `bm` on reverse** (`rotation_dir < 0`) — this is not decorative symmetry with direction, it fixes a real two-bike-wheels bug. The board is mounted flipped between a bike's two wheels: the front wheel's LED 0–43 face points at the disc rotor and forward spin reads correctly, but the rear wheel's frame/dropout geometry forces the board in facing the *cassette* instead, so it's the 44–87 face that ends up pointing at the disc rotor there — and that same physical flip reverses the Hall firing order, so `rotation_dir` reads `-1` on the rear wheel under completely normal forward travel. Before this fix, the 44–87 face always received `bf` (the angle that is only self-consistent for the 0–43 face), so on the rear wheel — viewed from the disc-rotor side, exactly the side that reads correctly on the front wheel — the image came out horizontally mirrored relative to the preview, even though both wheels were loaded with the identical file. Switching the shared reference to `bm` when `rotation_dir < 0` makes whichever face is measured to be spinning "backwards" (i.e. the one now facing the disc rotor on a flip-mounted wheel) self-consistent instead, at the cost of the *other* face now being the mirrored one — exactly as it already was on the normally-mounted wheel, just with the two faces' roles traded.
...
**The periodic `[HALL]` diagnostics (`cal`/`err`/`off`, `lead`/`jit`/`aT2`/`wcorr`, the per-sensor `e-raw` spread, `outer-pass`/`sem-take` timings, and the `Sensor N missed the magnet` skip log) were removed once the PLL, the adaptive-α gate and the per-arm calibration correction below had all been tuned and confirmed stable on hardware.** They were how the numbers quoted throughout this section were originally measured, and the algorithms they measured are unchanged — only the periodic printing (and the now-unused bookkeeping that fed it) is gone. Re-derive equivalent numbers from a debug build if retuning any of this is ever needed again.
...
- **CPU clock follows the same state, via `setCpuFreqForPower()`: 240 MHz only in `PWR_FULL`, 80 MHz everywhere else** (`PWR_OFF`/`PWR_SPINUP`, and once at the end of `setup()` before the first `loop()` pass). This build has `CONFIG_PM_ENABLE` off (no ESP-IDF dynamic frequency scaling or automatic light sleep) and `platformio.ini` pins `board_build.f_cpu` to 240 MHz, so without this the CPU would sit at full clock permanently — dwarfing anything BLE advertising power/interval tuning can do, since that only touches the brief TX bursts, not the constant CPU draw. 80 MHz is free everywhere outside rendering: on ESP32-S3, APB stays at 80 MHz whether the CPU multiplier is set to 80, 160 or 240 — only the CPU core multiplier changes — so SPI/DMA to the SK9822s, BLE and Wi-Fi (all clocked off APB, and all documented to want ≥80 MHz APB) run identically either way, and only CPU-bound work (`fillSectorIntoBuffer()`'s per-LED math, general code execution) is slower at 80 MHz. That's exactly the one place it must not be: `applyPowerState(PWR_FULL)` raises the clock as its *first* action, before `powerRailUpAndBlank()`, so the DMA fill/queue path never runs at the lower clock.
...
- **A deliberate burst of shakes also postpones that sleep, but a single knock does not.** `last_motion_ms` — the "confirmed activity" clock the idle countdown is measured against — used to advance only for a Play request or a wheel that actually spun up; the vibration sensor firing on its own never touched it, on purpose (see the comment at its declaration): counting every trigger would mean a wheel jostling in a bag never sleeps. That is still true for one or two knocks. But `vibrationInterruptHandler()` now also counts consecutive pulses (FALLING edge, one per physical knock — it used to be attached `CHANGE`, which double-counted) in a rolling `VIB_WAKE_WINDOW_MS` window, the same threshold (`VIB_WAKE_HITS_REQUIRED`, 20) that already distinguishes a deliberate shake from a single bump when waking the chip from deep sleep in the first place (see the vibration-sensor caveat below) — reaching it sets `vib_burst_event`, and `loop()` advances `last_motion_ms` on it, logging `[SYS] Vibration burst detected, sleep timer reset`. This runs unconditionally, not just in `PWR_OFF` like `wakeup_event`'s spin-up trigger, so it also extends the wait while `PWR_SPINUP` is already measuring RPM. The point: after the chip is woken by a 20-hit burst and is sitting there waiting for real rotation or a BLE connection, the *same* gesture repeated should read as "still trying to reach it," not silently do nothing until the 60 s/5 min timeout fires anyway.
...
**The slideshow plays a chosen subset of files *and* procedural effects.** `applySlideList(include, names, effectMask)` stores a filename list + a mode flag + a 6-bit effect mask: `include == false` — skip those names, `include == true` — play *only* those; `slideInSlideshow()` treats an empty *include* list as "no files" and an empty *exclude* list as "all files". `effectMask` bit N-1 adds `EffectId` N to the rotation. The advance loop in `loop()` builds a virtual sequence — set effects last, always in `1..6` order — and steps `slideCurrentIndex` through it; a file entry sets `pendingFilePath`, an effect entry sets `pending_effect` (the same `fileLoaderTask` path handles both, and `effectsStart` frees the outgoing file buffer / `loadFrameFromFile` calls `effectsStop`). **File order depends on the selection mode.** `exclude` mode (or no selection at all — "all files") has no explicit list to fall back on, so order comes from `savedFiles`, the raw order `updateFileList()` happens to enumerate LittleFS in — fine for a single device, since that's the only order it has ever had. `include` mode is different: order follows the *sent* list itself (filtered down to names that still exist), not `savedFiles`. This matters specifically for a synced group (see below): the phone sends the identical include list to every member, and `savedFiles` order is never guaranteed to match between two independently-provisioned wheels — building the sequence from `savedFiles` there turned one shared selection into a *different* playback order per wheel, which read as wheels showing outright different content, not gradually drifting out of phase. If the sequence is empty it holds the current frame instead of spinning. Persisted in NVS (`slidelist` string, `slidelistmode` + `slideeffmask` bytes), written by the same deferred `flush*()` path as settings and reloaded by `loadSlideList()` in `setup()` — survives sleep and a power cut; not in RTC (`setup()` rebuilds it like `savedFiles`). Set over BLE by the extended `OP_ALBUM` start payload (`POV_FEAT_ALBUM_SEL`, trailing `effMask` byte) or over the web by `/album?action=start&incl=…` / `&excl=…` / `&eff=1,3,5`.

**A synced group layers a live correction on top of that same on-device slideshow, instead of replacing it.** Two clocks, deliberately: the *soft* one is each wheel's own `advanceSlideshow()`, armed identically on every member (same include list — so same order, per above — same interval) and needing no phone at all once armed; the *hard* one is the phone (or `SyncSlideshowService` once the app is closed — see the Android app section), which is a better clock whenever it's actually there, and nudges each wheel onto the identical file/effect every interval via `OP_SYNC_TICK`. That opcode calls the same `syncTick()` that loads a file/effect exactly like `OP_PLAY`/`OP_EFFECT` do — *except* it leaves `slideshowActive` alone. `OP_PLAY`/`OP_EFFECT` clearing it is exactly right for a deliberate, one-off manual pick that should end any slideshow; a sync correction is the opposite — it's *reinforcing* the slideshow that's already meant to be running, so disarming it on every tick would mean the soft clock beneath it is never actually armed for longer than one interval. `syncTick()` still keeps the two clocks agreeing on *where* they are: it looks up the corrected item's position in the current sequence (`slideSequencePositionOf()`, the same sequence `advanceSlideshow()` walks) and sets `slideCurrentIndex` to it, and resets `slideLastSwitch` to now — so if the phone vanishes right after this tick, the next autonomous step continues from the right place instead of repeating it or skipping ahead. Net effect: hard sync whenever a phone is present, degrading gracefully to soft (each wheel's own oscillator, typically seconds/hour) the moment it isn't, with no seam at the boundary in either direction.
...
**`OP_SYNC_TICK` doesn't reset the BLE idle timer either, for the same reason.** `handleCmd()` treats most commands as activity (`last_web_activity_time = millis()`), deliberately excluding only `OP_TELE`/`OP_FRAG` — background polling that would otherwise mean "the device never sleeps while the app is merely open." `OP_SYNC_TICK` is exactly that same class of message: it arrives automatically once per slideshow interval for as long as a synced group session runs, including from `SyncSlideshowService` after the phone app itself has closed, with no human touching anything. It's excluded alongside `OP_TELE`/`OP_FRAG` at the opcode level.

**That alone wasn't enough — `syncTick()` itself was still faking a Play request underneath.** Whenever the corrected item differs from what's already loaded (the normal case: the slideshow is advancing on the phone's own clock every interval, so it almost always differs), `syncTick()` used to set `request_play_flag` exactly like a genuine `OP_PLAY`. That flag is not just "go load this file" — `fileLoaderTask()` reacts to `pendingFilePath`/`pending_effect` directly and needs no such flag at all; `request_play_flag`'s *only* consumer is `loop()`, which treats it as proof a human just asked to play something, and (1) stamps `last_motion_ms`/`last_play_ms` as confirmed activity and (2) sets `play_pending`, which raises DCDC 1 out of `PWR_OFF` into `PWR_SPINUP` to go measure RPM. A synced tick on a wheel that is sitting still triggered both, every interval: it kept trying to spin up arm 1 on a wheel nobody was riding, and — worse — `PWR_SPINUP` only falls back to `PWR_OFF` after `now_ms - last_play_ms > 10000` (10 s with *no* Play), so any slideshow interval under 10 s pinned `last_play_ms` fresh forever and the state machine never even reached `PWR_OFF` again, let alone the idle-sleep check that requires it. `syncTick()` now leaves `request_play_flag` alone — the content still loads exactly the same way through the semaphore — so a stationary member of a synced group actually reaches `PWR_OFF`, then sleeps after 5 minutes of no rotation, no vibration burst and no real command, same as any other BLE-connected wheel, and rejoins the group normally (vibration/rotation) if picked up again.

**Stopping the slideshow stops the display too.** `OP_ALBUM`(stop)/`/album?action=stop` used to only clear `slideshowActive`, leaving whatever file (or effect) was last showing keep playing — so stopping a slideshow took two presses: one to stop the cycling, another to actually blank the rim. That's exactly what `OP_STOP`/`/stop` already did (`slideshowActive = false; force_stop_display = true;`, plus clearing a running effect through `fileLoaderTask`), so the two are now the same call: `stopDisplayAndSlideshow()`. A synced group (Android app — see below) *does* set `slideshowActive` on-device on every member — that's the autonomous fallback layer, armed via ordinary `OP_ALBUM` — but the phone (or, once the app closes, `SyncSlideshowService`) additionally drives it live with `OP_SYNC_TICK`, a variant of "show this file/effect now" that deliberately does *not* touch `slideshowActive`, so the fallback stays armed underneath every live correction. Either way, `stopDisplayAndSlideshow()` on one wheel does nothing to stop the *others* cycling — Stop is tapped on only one screen. `WheelVm.stopSyncedSlideshow()` compensates from the app side: it sends `OP_STOP` to *every* wheel in the group (two or more), not just the one the user tapped Stop on.

**A single-item slideshow must not reload itself every interval.** With exactly one eligible file (or one eligible effect) in the selection, `advanceSlideshow()`'s `(index+1) % total` always lands back on the same item — but it used to still push a fresh `pendingFilePath`/`pending_effect` and re-trigger the load every single interval. `loadFrameFromFile()` blanks the strip for the whole read (see below), so a one-file slideshow blinked off and back on every interval to redisplay pixel-identical content; a one-effect slideshow similarly reset the effect's internal state (`fire_heat`, `eff_read`, clock hands) on every "switch" that wasn't actually a switch. `advanceSlideshow()` now skips the reload/restart when the computed next file equals `currentDisplayFile`, or the next effect equals the already-running `effect_id`.

**Deleting the file currently on the rim doesn't reach the renderer by itself.** `OP_DELETE`/`/delete` only ever did `LittleFS.remove()` — the decoded frame already lives in PSRAM, so the strip would keep looping a file that no longer exists on flash. `handleFileDeleted(fname)` (called right after the remove, from both transports, with the leading `/` as stored in `currentDisplayFile`) checks whether the removed name was the one actually showing; if so it doesn't do the work itself. Freeing the buffer needs the same gash-and-wait as any other frame swap (`render_in_fill`/`rendering_active`, up to ~1.2 s), and doing that synchronously inside the BLE callback or the HTTP handler would block the NimBLE host task or the async server for that whole stretch — the same supervision-timeout risk flash writes already avoid by going through a separate task. So it only sets a flag (`pending_unload`) and wakes `fileLoaderTask`, exactly like `OP_PLAY` hands its own load off to the same task instead of calling `loadFrameFromFile` inline. `handleActiveFileUnload()` there calls `unloadCurrentFrame()` (same free-and-null as effects already do — `fillSectorIntoBuffer` treats a null `frameBuffer` as "blank everything", so there's no separate "nothing loaded" state to invent) and, if the file was part of the running slideshow, advances to whatever came next in the sequence — `advanceSlideshow()` is the exact same "pick the next file or effect" logic `loop()` uses for a timed switch, factored out so deleting the active file doesn't wait out `slideInterval` first. Rewinding `slideCurrentIndex` by one before calling it compensates for the `+1` inside `advanceSlideshow()` and for `updateFileList()` having just closed the gap left by the removed entry — the file that used to sit right after it now sits at that same index. If nothing is left to advance to (`advanceSlideshow()` returns false — the deleted file was the only thing in rotation), the slideshow stops instead of looping on nothing.
...
(The per-sensor skip counters and the `[HALL] Sensor N missed the magnet` log that used to surface this were removed along with the rest of the `[HALL]` diagnostics — see the note at the end of this section.)
...
**TX power (`ESP_PWR_LVL_P9`, +9 dBm) and the 100–150 ms advertising interval
are not the dominant idle current cost, measured on real hardware.** Both were
lowered first (down to 0 dBm and 300 ms) on the reasonable-looking theory that
advertising bursts were the ~130 mA seen with no phone connected — current on
the wheel didn't move at all, which is what actually pinned the real cause down
to the fixed 240 MHz CPU clock (see `setCpuFreqForPower()` under Power
Management below): this build has no ESP-IDF dynamic frequency scaling, so the
CPU ran at full clock permanently regardless of what BLE was doing, dwarfing
anything advertising tuning could touch. Once the clock scaling fix dropped the
reading to ~100 mA, P9/100–150 ms were put back — they cost comparatively
little next to the CPU, and buy back range and near-instant discovery.
...
- **Idle deep sleep wakes on EXT1 `ANY_LOW`, mask `{IO0, IO15}`** — a shake (the vibration sensor pulses IO15 LOW) *or* a single button click (IO0 LOW). Level-triggered, so `enterDeepSleep()`/`enterTrickleSleep()` first spin ≤2 s for both pins to read HIGH. `esp_sleep_get_ext1_wakeup_status()` says which pin fired, for the log. `user_wake` (`xport_wake || wakeup == EXT1`) means a human woke it — used to skip the on-USB trickle re-check. Transport sleep still uses EXT0 on IO0 alone (single-pin, and by design no vibration). Both DCDC enables are `gpio_hold_en`'d LOW before sleeping.
- **A vibration sensor still LOW after that 2 s window is not dropped from the wake mask forever.** It almost never means the switch is broken — far more often the wheel is simply resting in an orientation where the ball sits on the contact (on its side, say), which can persist for as long as it sits there. Excluding it outright would mean "always wakes by shake unless put to sleep by a button hold or app Off" stops being true the moment the wheel is stored on the wrong side — silently, since the exclusion used to be a single log line nobody sees while the device is already asleep. An earlier revision handled this with a periodic timer (`VIB_STUCK_RECHECK_S`, 3 s) waking the chip briefly to poll the pin — simple, but expensive: each poll is a full exit from deep sleep for up to ~150 ms of active current, and on a reasonable interval that averaged out to roughly 1 mA continuously for as long as the wheel sat in that orientation — enough to measurably drain the battery over days on a shelf, and the actual complaint that prompted replacing it. `armWakeSourcesAndSleep(false)` now instead arms the button alone on EXT1 (`ANY_LOW`, single-pin mask) and the vibration pin on **EXT0 with the *opposite* level** (`HIGH`) — the chip doesn't wake at all until the ball actually rolls off the contact, and does so the instant it happens, via dedicated wakeup hardware, not a poll. EXT1 and EXT0 are used together specifically because EXT1's polarity is one setting for its whole pin mask — it cannot watch the button for LOW and the vibration pin for HIGH in the same call — while EXT0 configures a single pin's level independently. The cost: EXT0 wakeup requires the RTC peripherals power domain to stay powered for as long as it's armed (unlike EXT1, which doesn't), raising sleep current from ~10 µA to on the order of ~50–100 µA while the sensor is stuck — an order of magnitude cheaper than the old polling average, and it no longer scales with how long the wheel stays in that pose. `vib_stuck_wait` (RTC memory) lets `setup()` tell this apart from a real wake: woken by EXT0 while it's set, it re-debounces the pin with a short `waitPinReleased()` (20 ms stable, `VIB_UNSTUCK_DEBOUNCE_MS` cap — cheap on purpose, contact bounce right at the threshold is the only thing this filters, since the hardware wakeup itself already proves the level went HIGH). Still LOW: straight back to the same EXT0-armed sleep — no flash writes, no LittleFS/BLE/Wi-Fi init. **Released: it does *not* fall through to a normal boot.** An earlier version did, and that was the actual bug behind "sometimes doesn't wake by shake": a full boot idles for up to a minute before its own `enterDeepSleep()` re-tries the vibration debounce, and if the wheel is simply resting in a pose where the ball settles back onto the contact (a stable storage position, not a one-off bounce), that later debounce fails again — button-only, forever, in a loop that looks from the outside like "shake never wakes it," even though the recheck genuinely woke the board each time. Instead, a released sensor goes straight into `armWakeSourcesAndSleep(true)` — vibration is back on EXT1 `ANY_LOW` and the chip returns to sleep immediately, no boot in between. If the release was itself the tail of the shake that triggered the wakeup, the very next pulse (or the ball not yet having resettled) wakes it through ordinary EXT1 a moment later; if the sensor is genuinely free and stays that way, the chip is now correctly armed for the *next* shake instead of only for the one it happened to catch.
- **A single vibration pulse does not fully wake the device — `VIB_WAKE_HITS_REQUIRED` (20) consecutive ones do.** A real EXT1 wake used to always fall straight into a full boot (BLE, LittleFS, at least a few seconds of activity before it can decide nothing is happening and sleep again), and a wheel in a bag or on a rack takes plenty of single, unrelated knocks that were never meant to wake it — each one paid for that whole cycle. A deliberate "wake the wheel" shake, by contrast, is never a single clean pulse — a hand can't hold it still, so it produces several pulses in quick succession. `setup()` now counts consecutive EXT1 wakes caused by the vibration pin alone (`vib_wake_hits`, RTC memory) and only lets the `VIB_WAKE_HITS_REQUIRED`-th one fall through to a real boot; every one before that takes the same cheap path as the stuck-sensor recheck above — a short `waitPinReleased()` debounce, then straight back into `armWakeSourcesAndSleep()` (which itself picks EXT1-both-pins or the EXT0-stuck-wait arm depending on what it finds) — without ever touching BLE, Wi-Fi or LittleFS. A wake caused by the button (alone or together with vibration) always counts as real immediately and resets the counter, as does reaching the threshold. **The count is also windowed in time** (`VIB_WAKE_WINDOW_MS`, 1 s): if more than that has passed since the previous counted pulse (`vib_wake_last_ms`, RTC memory, in `gettimeofday()` milliseconds — not `millis()`, which resets on every deep sleep and can't measure a gap *between* sleeps), this pulse restarts the count at one instead of continuing it, so pulses from unrelated events on different days can no longer add up into a spurious boot. **The counter is bypassed entirely while `batt_trickle_mode` is set**: a shake is the only way to reach a device sitting on trickle charge with a near-dead cell (see Trickle-Charge Recovery below), so requiring five of them there would partly defeat the escape hatch.
