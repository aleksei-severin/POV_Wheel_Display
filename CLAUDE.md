...
- **A vibration sensor still LOW after that 2 s window is not dropped from the wake mask forever.** It almost never means the switch is broken — far more often the wheel is simply resting in an orientation where the ball sits on the contact (on its side, say), which can persist for as long as it sits there. Excluding it outright would mean "always wakes by shake unless put to sleep by a button hold or app Off" stops being true the moment the wheel is stored on the wrong side — silently, since the exclusion used to be a single log line nobody sees while the device is already asleep. `armWakeSourcesAndSleep(false)` arms the button alone plus a bare timer wakeup every `VIB_STUCK_RECHECK_S` (3 s, `config.h` — was 20 s; the longer period made the self-heal correct but slow enough that a quick "shake it and see" test looked exactly like "still broken", so it was traded down: worse idle-shelf current if the wheel is truly resting on the contact for a long stretch, but that is a rarer cost than the wake path silently failing every time someone actually tries it) instead, and sets `vib_stuck_recheck` (RTC memory) so `setup()` can tell a routine recheck apart from a real wake: woken by that timer, it re-debounces the pin with a short `waitPinReleased()` (20 ms stable, 150 ms cap — cheap on purpose, this runs every `VIB_STUCK_RECHECK_S`). Still LOW: straight back to sleep the same way — no flash writes, no LittleFS/BLE/Wi-Fi init, just the GPIO hold + `esp_sleep_enable_ext1_wakeup`/`esp_sleep_enable_timer_wakeup` pair. **Released: it does *not* fall through to a normal boot.** An earlier version did, and that was the actual bug behind "sometimes doesn't wake by shake": a full boot idles for up to a minute before its own `enterDeepSleep()` re-tries the vibration debounce, and if the wheel is simply resting in a pose where the ball settles back onto the contact (a stable storage position, not a one-off bounce), that later debounce fails again — button-only, forever, in a loop that looks from the outside like "shake never wakes it," even though a lucky recheck genuinely woke the board each time. Instead, a released sensor goes straight into `armWakeSourcesAndSleep(true)` — vibration is back in the real wake mask and the chip returns to sleep immediately, no boot in between. If the release was itself the tail of the shake that triggered the recheck, the very next pulse (or the ball not yet having resettled) wakes it through ordinary EXT1 a moment later; if the sensor is genuinely free and stays that way, the chip is now correctly armed for the *next* shake instead of only for the one it happened to catch.
