...
**The clock is now a live setting, not just a boot-time constant.** `SK9822_SPI_HZ` still supplies the initial divider (`global_spi_div`, default n=4 → 20 MHz), but the web UI (Display → SPI clock) exposes the whole reachable ladder `f = 80 MHz / n` for `n` = 2…64 (40 MHz down to 1.25 MHz) as a slider over `n` itself — not over MHz, since nothing exists *between* rungs of that ladder and a linear-MHz slider would have dead positions. `POST /settings?spidiv=n` only *requests* the change (`pending_spi_div`); `applySpiDiv()` does the actual `spi_bus_remove_device`/`spi_bus_add_device` (clock speed can't change on a live device any other way) from `loop()`, and only after `spi_reconfig_pending` has made `renderingTask` blank the strip and back off the same way `frame_loading` does — touching `sk9822_spi` while a transaction could still be in flight is exactly the DMA-queue hazard already worked out for `blankAllLEDs_DMA()` (see the Hall/lead debugging history below). The result feeds `g_dma_frame_us`, which `renderingTask` re-reads once per outer-loop pass (same cadence as `arm_reverse_seen`) instead of the old compile-time `SK9822_FRAME_US` — a plain `volatile float` read is atomic on this architecture, so no lock is needed. Persisted in `SettingsBlob` version 5 (`spi_div`, range-checked 2–64 on load).

Exposed on BLE too, same idempotent-guard pattern as `arm_trim`: `PovSettings.spi_div` (uint8, appended to the struct's end — `static_assert(sizeof(PovSettings) == 39, ...)` in `povble.cpp` guards it against `Settings.SIZE` in `Proto.kt` the same way it already does for `arm_trim_x10`) only sets `pending_spi_div` when the incoming value actually differs from `global_spi_div`, so re-sending the whole settings blob (every slider commit does, `armTrim` included) can't re-trigger a bus reconfigure for a value that didn't change. Mirrored as `Settings.spiDiv: Int` in `Proto.kt`, with a matching slider in `DeviceScreen.kt` (`SpiClockControl`, same n=2..64 range).
...
**`global_arm_trim[NUM_ARMS]` is a manual, per-arm angle trim on top of `rtc_hall_cal[]`, and the two are not redundant.** Hall calibration measures only where the *sensor* is (it's coaxial with the arm's centre) — it cannot see, and by construction never will, how accurately the *LED board itself* is seated on that arm. That mounting tolerance is invisible to Hall timing entirely (nothing about it changes when the sensor fires), yet it produces exactly the same symptom as the bug above: a static, RPM-independent seam, identical every revolution, worse toward the rim. Confirmed on hardware before adding this: `[HALL] e-raw` (the *raw*, pre-EMA calibration measurement, logged separately from the converged `rtc_hall_cal[]`) sat within 0.1–0.3° of the converged value on every single sample — so the Hall measurement itself is precise and not the source of a larger residual — while the seam actually visible on the wheel measured ~2° (~10 mm at the 273 mm rim), well beyond anything `rtc_hall_cal[]` (max ~1°) could account for even if misapplied. `fillSectorIntoBuffer()` applies it the same way and with the same sign as the Hall offset: `bf = sector0 + ray·arm_step − rtc_hall_cal[ray] − global_arm_trim[ray]`. It is `RTC_DATA_ATTR`, persisted in `SettingsBlob` (version 4, `arm_trim[NUM_ARMS]`, defaults to 0 on older blobs), and set per arm from **both** UIs — there is no automatic way to derive it, since nothing about the sensor timing carries this information; it's tuned by eye or from a photo against still content until the seam disappears:
- **Web**: Display → Arm trim, six sliders, `/settings?t0=..&t1=..` etc. (`GET /get_settings` echoes it back as `"trim":[...]`), degrees, range ±15°.
- **BLE/Android**: `PovSettings.arm_trim_x10[6]` (int16, ×10, appended to the *end* of the struct so old firmware/app pairs fail loudly instead of silently misreading fields — `povble.cpp` has a `static_assert(sizeof(PovSettings) == 38, ...)` against `Settings.SIZE` in `Proto.kt` for exactly this reason, and both must be bumped together). Mirrored as `Settings.armTrimX10: List<Int>` in `Proto.kt`, with a matching "Arm trim" card in `DeviceScreen.kt` (six sliders, same ±15° range, 0.1° steps).
...
- **PSRAM, not flash, caps animation length.** The whole animation is resident, so ~8 MB of PSRAM ÷ 16,608 B ≈ **480 frames ≈ 48 s at 10 fps**, still well under the 13 MB LittleFS partition, which holds ~820 frames in total across all files. `/fs_info` reports `psram_free` so the UI can cap frame count and warn *before* an upload that would only fail at play time. Streaming frames from flash instead is not an option — see the blanking note above.
