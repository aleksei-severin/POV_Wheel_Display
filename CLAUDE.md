...
**Stopping the slideshow stops the display too.** `OP_ALBUM`(stop)/`/album?action=stop` used to only clear `slideshowActive`, leaving whatever file (or effect) was last showing keep playing — so stopping a slideshow took two presses: one to stop the cycling, another to actually blank the rim. That's exactly what `OP_STOP`/`/stop` already did (`slideshowActive = false; force_stop_display = true;`, plus clearing a running effect through `fileLoaderTask`), so the two are now the same call: `stopDisplayAndSlideshow()`. A synced group (Android app — see below) never sets `slideshowActive` on-device at all: the phone (or, once the app closes, `SyncSlideshowService`) drives it with explicit, repeated `OP_PLAY`/`OP_EFFECT` — not `OP_ALBUM` — because each device's own on-device sequence order comes from `savedFiles`, the raw order LittleFS happens to enumerate files in, which is never guaranteed to match between two independently-provisioned wheels; naming the exact file/effect from the phone sidesteps that per-device ordering entirely. So `stopDisplayAndSlideshow()` on one wheel does nothing to stop the others cycling. `WheelVm.stopSyncedSlideshow()` compensates from the app side: it sends `OP_STOP` to *every* wheel in the group (two or more), not just the one the user tapped Stop on.
