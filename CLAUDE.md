...
**Stopping the slideshow stops the display too.** `OP_ALBUM`(stop)/`/album?action=stop` used to only clear `slideshowActive`, leaving whatever file (or effect) was last showing keep playing — so stopping a slideshow took two presses: one to stop the cycling, another to actually blank the rim. That's exactly what `OP_STOP`/`/stop` already did (`slideshowActive = false; force_stop_display = true;`, plus clearing a running effect through `fileLoaderTask`), so the two are now the same call: `stopDisplayAndSlideshow()`. A synced group (Android app, phone-driven — see below) needs the same guarantee from the app side instead, since a phone-driven group never sets `slideshowActive` on-device at all: `WheelVm.stopSyncedSlideshow()` sends `OP_STOP` to *every* wheel in the group (two or more) before tearing down the sync, not just the one the user tapped Stop on.
