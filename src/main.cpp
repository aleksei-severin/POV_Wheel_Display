...
static const uint8_t  SETTINGS_VER   = 5;
...
static void loadSettingsFromNVS() {
    ...
    bool ok = (b.magic == SETTINGS_MAGIC) &&
              ((got == sizeof(b)          && b.version == SETTINGS_VER) ||
               (got == SETTINGS_V4_SIZE   && b.version == 4) ||
               (got == SETTINGS_V3_SIZE   && b.version == 3) ||
...
static void fillSectorIntoBuffer(uint8_t* buf, uint8_t buf_idx, float sector0, float span) {
    ...
    for (int ray = 0; ray < NUM_ARMS; ray++) {
        ...
        // mirror_back_face:
        //   true  → сторона диодов 44–87 берёт зеркальный bm, сторона 0–43 (та,
        //     что «со стороны первого диода») — прямой bf. Так текст Speed/Clock
        //     и анимации с галочкой читается с обеих сторон колеса, ВСЕГДА,
        //     независимо от направления вращения ниже: bf/bm — это свойство
        //     геометрии диода (каждая сторона по отдельности читается верно
        //     сама с собой), а не того, какая сторона физически смотрит на
        //     наблюдателя, так что мешать сюда rotation_dir не нужно.
        //     Проверено на железе: часы и скорость выходили зеркальными сразу с
        //     обеих сторон — это значит, что была перепутана не одна сторона, а
        //     сама пара ролей (обе стороны получали не ту половину), поэтому
        //     чинится именно обменом bf/bm местами, а не правкой одной стороны.
        //   false → обе стороны берут ОДИН и тот же угол-эталон: горят одни и
        //     те же пиксели, изнанка читается зеркально (то, что нужно
        //     симметричной картинке/анимации). Эталон — bf при прямом вращении,
        //     но bm при обратном. Это не декоративная симметрия: на велосипеде
        //     с двумя колёсами плата физически развёрнута между ними — переднее
        //     колесо стоит платой (стороной 0–43) к тормозному диску, а заднее
        //     тем же диском конструктивно смотрит на кассету, так что к
        //     тормозному диску там повёрнута уже сторона 44–87 — и та же самая
        //     механическая разница между колёсами меняет знак rotation_dir
        //     (порядок срабатывания датчиков зеркален), так что направление
        //     служит готовым признаком «эта плата стоит развёрнуто» без
        //     отдельной настройки на каждое колесо. Без этой развязки сторона
        //     44–87 при обратном вращении всегда получала бы bf — верный угол
        //     для ЧУЖОЙ (0–43) стороны — и картинка, снятая с тормозного диска
        //     заднего колеса, выходила отзеркаленной относительно превью и
        //     относительно переднего колеса, хотя обе платы залиты одним и тем
        //     же файлом.
        float ref = (rotation_dir < 0) ? bm : bf;
        float a_f = ref, a_b = ref;
        if (mirror_back_face) { a_f = bf; a_b = bm; }
...
// поломка, а чаще просто положение колеса — см. armWakeSourcesAndSleep()
// ниже и VIB_UNSTUCK_DEBOUNCE_MS в config.h), кнопка не должна оставаться
// единственным будильником НАВСЕГДА. Вместо периодического таймера чип в
// этом случае вооружает EXT0 на самом вибродатчике будить по ВЫСОКОМУ
// уровню — т.е. не встаёт вообще, пока шарик не скатится с контакта, и
// просыпается аппаратно ровно в момент этого события. setup() видит этот
// флаг и, если разбудил именно EXT0, коротким дебаунсом отсеивает дребезг
// на границе контакта: всё ещё зажат — тихо ложится обратно тем же путём
// (EXT0 по HIGH), не поднимая систему целиком. Отпустило — вибрация
// немедленно возвращается в обычную маску пробуждения (EXT1, ANY_LOW) и чип
// сразу же ложится спать СНОВА, не поднимая систему: это не настоящее
// пробуждение человеком, а починка будильника, и полный подъём (BLE,
// LittleFS...) ради неё не нужен. Настоящее пробуждение (кнопка, либо
...
// с равным шансом попадает в фазу чужого дребезга; этот цикл — нет.
static bool waitPinReleased(uint8_t pin, uint32_t stable_ms, uint32_t timeout_ms) {
    uint32_t t0 = millis(), high_ms = 0;
    while (millis() - t0 < timeout_ms && high_ms < stable_ms) {
        high_ms = (digitalRead(pin) == HIGH) ? high_ms + 5 : 0;
        delay(5);
    }
    return high_ms >= stable_ms;
}

// Общий финальный шаг ухода в обычный сон по простою: держит DCDC в LOW и
// вооружает источники пробуждения. Не трогает флеш — можно звать повторно
// сколько угодно раз подряд (см. vib_stuck_wait выше), не изнашивая её.
//
// arm_vib == true  — обычный случай (вибродатчик отпущен): кнопка и
//   вибродатчик вместе на EXT1, ANY_LOW. EXT1 не требует включённой
//   RTC-периферии — это и даёт честные ~10 мкА сна.
// arm_vib == false — вибродатчик зажат на LOW прямо сейчас. Вместо того
//   чтобы будить чип раз в несколько секунд опросом (дорого, см.
//   VIB_UNSTUCK_DEBOUNCE_MS в config.h), вооружаем EXT0 именно на этом пине
//   будить по ПРОТИВОПОЛОЖНОМУ, высокому уровню — аппаратное пробуждение
//   сработает ровно в момент, когда шарик скатится с контакта, без всякого
//   опроса. Кнопка остаётся будильником отдельно на EXT1 (в одиночной
//   маске) — у EXT1 одна полярность на всю маску сразу, смешать в ней LOW
//   для кнопки и HIGH для вибродатчика нельзя. Пока EXT0 вооружён, RTC-
//   периферия обязана быть включена (иначе не на чем сравнивать уровень) —
...
static void armWakeSourcesAndSleep(bool arm_vib) {
    ...
    if (arm_vib) {
        ...
        uint64_t wake_mask = (1ULL << PIN_BUTTON) | (1ULL << PIN_VIBRATION);
        esp_sleep_enable_ext1_wakeup(wake_mask, ESP_EXT1_WAKEUP_ANY_LOW);
    } else {
        vib_stuck_wait = true;
        esp_sleep_enable_ext1_wakeup((1ULL << PIN_BUTTON), ESP_EXT1_WAKEUP_ANY_LOW);
        esp_sleep_enable_ext0_wakeup((gpio_num_t)PIN_VIBRATION, HIGH);
    }
...
static void enterDeepSleep() {
    ...
    if (!vib_ok) {
        webLog("[SYS] Vibration sensor stuck LOW at sleep entry, waiting for release via EXT0");
    }
...
// =====================================================================
//  Мостики для BLE (объявлены в povble.h)
//
//  Кеш телеметрии питания и коэффициенты самокалибровки — static в этом файле,
//  и вытаскивать их в глобальные ради одной команды BLE не за чем: у снимка
//  ровно один потребитель и ровно одна точка сборки.
// =====================================================================
void povGetPowerTele(PovPowerTele* out) {
    if (!out) return;
    out->vbat_mv = pwr_cache.vbat_mv;
    out->vusb_mv = pwr_cache.vusb_mv;
    out->ocv_mv  = pwr_cache.ocv_mv;
    out->soc     = pwr_cache.soc;
    out->chg     = pwr_cache.chg;
    out->usb     = pwr_cache.usb ? 1 : 0;
    // Отдаём то, к чему сошлась самокалибровка: по этим двум числам
    // проверяют модель заряда — ЭДС не должна дёргаться при включении ленты.
    out->sag_mv  = (int16_t)batt_sag_k;
    out->rise_mv = batt_chg_rise;
}

// Уснуть по команде из приложения. Не засыпаем прямо здесь: сон — это флеш
// (сброс настроек и калибровки Холла) и гашение ленты, а вызывают нас из
// задачи хоста NimBLE, которой в этот момент ещё отвечать телефону.
// Вместо этого состариваем таймеры простоя, и loop() уходит в сон своим
// штатным путём — со всеми проверками, которые в нём уже написаны.
//
// last_hall_time НЕ трогаем ни при каких обстоятельствах: это метка реального
// события датчика, и ложь в ней сходит за вращение (см. CLAUDE.md — на этом
// уже один раз погорел слайдшоу, крутивший файлы на неподвижном колесе).
void povRequestSleep() {
    uint32_t now = millis();
    const uint32_t aged = 6UL * 60UL * 1000UL;   // старше самого щедрого порога простоя
    last_motion_ms         = (now > aged) ? (now - aged) : 0;
    last_web_activity_time = (now > aged) ? (now - aged) : 0;
}
...
void setup() {
    ...
    // --- Ожидание отпускания "залипшего" вибродатчика: до всего остального ---
    // Если разбудил именно EXT0 на вибродатчике, вооружённый по HIGH (см.
    // armWakeSourcesAndSleep) — это не пробуждение человеком, а сработавшее
    // условие "шарик скатился с контакта". Раньше здесь стоял периодический
    // таймер и полагаться на "повезло поймать момент" уже не нужно — EXT0
    // будит аппаратно ровно в момент события, но короткий дебаунс всё равно
    // нужен: граница контакта дребезжит, и мгновенный HIGH может тут же
    // вернуться в LOW.
    //   • всё ещё дребезжит / снова зажат — ложимся обратно тем же путём
    //     (EXT0 по HIGH), ничего не инициализируя;
    //   • отпустило по-настоящему — тут же (не поднимая систему) возвращаем
    //     вибро в обычную маску будильника (EXT1, ANY_LOW) через
    //     armWakeSourcesAndSleep(true) и снова засыпаем. Никакого полного
    //     boot ради этого не нужно: следующий же толчок (в том числе тот,
    //     которым это и проверяли) разбудит по-настоящему через EXT1.
    // В режиме ожидания кнопка сидит отдельно на EXT1 (см.
    // armWakeSourcesAndSleep) — пробуждение по EXT1 здесь однозначно кнопка,
    // и идёт обычным путём ниже без изменений.
    if (vib_stuck_wait) {
        vib_stuck_wait = false;
        if (wakeup_reason == ESP_SLEEP_WAKEUP_EXT0) {
            if (waitPinReleased(PIN_VIBRATION, 20, VIB_UNSTUCK_DEBOUNCE_MS)) {
                armWakeSourcesAndSleep(true);    // починка маски, не возвращается
            } else {
                armWakeSourcesAndSleep(false);   // всё ещё зажат, не возвращается
            }
        }
    }
    ...
    // непрерывного блока такого размера может уже не найтись. Сам стек
...
void loop() {
    ...
    // Простой усыпляет устройство и на кабеле тоже. Раньше USB сон отменял, и
    // это стоило дорого с обеих сторон:
    //   • пока идёт заряд, ESP32 съедает 100 мА из тех, что зарядник отдавал бы
    //     ячейке (в предзаряде это вообще весь его ток — см. enterTrickleSleep);
    //   • когда IP2312U отрапортовал «заряжено» и ток снял, те же 100 мА идут
    //     уже ИЗ батареи. Пакет разряжается до порога повторного старта,
    //     заряжается снова — и так по кругу, наматывая циклы впустую.
    ...
    // Тот же путь, что у удержания кнопки, только без кнопки: колесо гаснет и
    // до её удержания уже не проснётся. Исполняем здесь, а не в задаче хоста
    // NimBLE, — enterTransportSleep() пишет флеш и не возвращается.
    if (pending_transport_off) {
        pending_transport_off = false;
        webLog("[XPORT] Power off over BLE");
    ...
    // --- BLE: телеметрия и дожатие отложенных операций ---
    // Последним в проходе: телеметрия уезжает с теми же числами, что
    // updatePowerTelemetry() положил в кеш строкой выше, а не на проход старее.
    bleLoop();
}
