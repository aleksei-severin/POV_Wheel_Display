...
// setup() видит этот флаг и, если разбудил именно наш короткий таймер, тут же
// перепроверяет датчик. Всё ещё зажат — тихо ложится обратно тем же путём, не
// поднимая систему целиком. Отпустило — вибрация немедленно возвращается в
// маску пробуждения и чип сразу же ложится спать СНОВА, не поднимая систему:
// это не настоящее пробуждение человеком, а починка будильника, и полный
// подъём (BLE, LittleFS...) на минуту ради неё не нужен — а держать его
// висящим только на кнопке ещё VIB_STUCK_RECHECK_S лишний раз означало бы
// пропустить как раз тот толчок, ради которого перепроверка вообще существует.
// Настоящее пробуждение (кнопка, либо вибрация уже ПОСЛЕ того как маску
// починили) проходит эту проверку насквозь и обрабатывается как обычно.
...
void setup() {
    ...
    // Если разбудил именно наш короткий таймер (см. armWakeSourcesAndSleep) —
    // это не пробуждение человеком, а рутинная перепроверка. Раньше "отпустило"
    // означало провалиться дальше в полный boot (BLE, LittleFS, минута простоя)
    // просто чтобы ПОТОМ, на следующем обычном enterDeepSleep(), заново
    // попробовать включить вибро в маску — а если колесо просто лежит в
    // положении, где шарик естественно скатывается на контакт (не разовый
    // дребезг, а постоянная поза хранения), тот следующий вход в сон видел
    // датчик снова прижатым и исключал его снова: колесо крутилось по кругу
    // "иногда повезёт поймать момент между тряской" → минута бесполезного
    // бодрствования → тот же самый залип обратно, и по факту оно НЕ
    // просыпалось по тряске, что бы пользователь ни делал. Поэтому здесь сразу
    // лёгкий дебаунс (короче входного в enterDeepSleep — сама перепроверка
    // должна стоить миллисекунды, а не долю секунды) и по нему:
    //   • всё ещё зажат — ложимся обратно на тот же срок, ничего не
    //     инициализируя, как и раньше;
    //   • отпустило — тут же (не поднимая систему) возвращаем вибро в маску
    //     будильника через armWakeSourcesAndSleep(true) и снова засыпаем.
    //     Никакого полного boot ради этого не нужно: следующий же толчок
    //     (в том числе тот, которым это и проверяли) разбудит по-настоящему
    //     через EXT1, вместо того чтобы ждать, пока повезёт поймать момент,
    //     а колесо тем временем успеет снова лечь на контакт.
    // Разбудило что-то другое (кнопка) — идём обычным путём ниже без изменений.
    if (vib_stuck_recheck) {
        vib_stuck_recheck = false;
        if (wakeup_reason == ESP_SLEEP_WAKEUP_TIMER) {
            if (waitPinReleased(PIN_VIBRATION, 20, 150)) {
