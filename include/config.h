...
// Радиальная компенсация яркости, 0–100 %.
// Диод на радиусе r за оборот размазывает свой поток по длине 2πr, поэтому
// воспринимаемая яркость падает как 1/r: край колеса кажется в 49/273 = 0.18 раза
// тусклее ступицы. Компенсация гасит ближние диоды: gain = (r / r_outer)^(rad/100).
// 0 = выключено (как раньше), 100 = полная физическая компенсация.
extern volatile float global_radial_gain;
