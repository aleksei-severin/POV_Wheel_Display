...
// PovHello.name вместе с завершающим нулём. В рекламный пакет (31 байт, из
// которых 18 занимает 128-битный UUID сервиса) такое имя целиком не входит, но
// это не беда: NimBLE переносит не влезающее имя в scan response, а телефон
// при активном сканировании читает и его.
#define POV_NAME_MAX  19
...
struct PovSettings {
    ...
    // Добавлено вместе с web-версией: ручная подстройка угла каждого луча
    // поверх авто-калибровки Холла (global_arm_trim[]) — та видит только
    // положение датчика, а не то, насколько точно на луч посажена сама плата
    // со светодиодами. ±15°, x10. Дописано в конец структуры — старые клиенты
    // читают/пишут её без этого поля (SIZE проверяется на обеих сторонах).
    int16_t  arm_trim_x10[6];  // NUM_ARMS фиксировано железом, см. config.h
    // Делитель частоты SPI, f = 80 МГц/spi_div, диапазон 2…64 (40…1.25 МГц) —
    // драйвер умеет только этот дискретный ряд (pre=1), см. SK9822_SPI_HZ в
    // config.h. Смена живая: applySpiDiv() в main.cpp останавливает рендер на
    // время переинициализации SPI-устройства.
    uint8_t  spi_div;
};
...
// Заявка на уход в транспортный режим (питание off, пробуждение только
// удержанием кнопки). Ставится командой OP_POWEROFF, исполняется loop():
// enterTransportSleep() гасит ленту через SPI и пишет флеш, а в задаче хоста
// NimBLE этому не место — стирание страницы оборвёт связь по супервизии.
extern volatile bool pending_transport_off;
